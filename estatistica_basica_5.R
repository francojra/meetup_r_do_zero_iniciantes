...
# Os dados apresentam significante não normalidade (p<0.0032)
...
shapiro.test(a) # p>0.05 = paramétrico
hist(a)
shapiro.test(b) # p>0.05 = paramétrico
hist(b)

cor.test(a, b, method = "pearson")
plot(a, b)
abline(lm(a ~ b)) # Acrescenta linha ao gráfico
...
shapiro.test(x) # p<0.05 = não paramétrico
shapiro.test(y) # p<0.05 = não paramétrico
